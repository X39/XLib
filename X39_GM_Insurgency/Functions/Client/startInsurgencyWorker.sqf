...
[] spawn {
	...
	while{true} do
	{
		...
		_insObjects = nearestObjects [position player, ["Land_X39_GM_Insurgency_Module"], X39_GM_Insurgency_var_spawnRange];
		...
		DEBUG_LOG("InsurgencyWorker: Validating spawned units")
		{
			if(typeName _x != "SCALAR") then
			{
				...
				else
				{
					...
					if(typeOf _x != "Land_X39_GM_Insurgency_Module") then
					{
